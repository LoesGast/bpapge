...
def drop_tables():
    tables = ["Genes", "Protein", "Exon", "Region", "Sites", "Protein_EC_Nummer", "EC_nummer", "Pathways", "EC_Pathway"]
    for item in tables:
        print(item)
def create_tables():
    ...
    cursor.execute(SQL_Exon)
    cursor.execute(SQL_Region)
    cursor.execute(SQL_Sites)
    cursor.execute(SQL_Protein_Ec_nummer)
    cursor.execute(SQL_EC_Nummer)
    cursor.execute(SQL_Pathways)
    cursor.execute(SQL_Ec_nummer_Pathway)
    conn.commit()


def main():
    # Define our connection string
    conn_string = "host='localhost' dbname='a1_tijger' user='hanna' password='Hanna'"
